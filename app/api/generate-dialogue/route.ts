...
export async function POST(request: Request) {
  try {
    ...
    let productAnalysis, smartStrategy
    try {
      console.log("🔍 Analyzing product with smart system...")
      productAnalysis = analyzeProduct(productName, productDescription)
      smartStrategy = generateSmartDialogueStrategy(productAnalysis, productName)
      ...
      smartStrategy = {
        hook: `Bạn có đang tìm kiếm ${productName}?`,
        problemStatement: `Nhiều người đang gặp khó khăn trong việc tìm sản phẩm chất lượng...`,
        solutionPresentation: `${productName} chính là giải pháp hoàn hảo!`,
        proofPoints: ['Được nhiều khách hàng tin dùng', 'Chất lượng đảm bảo'],
        emotionalConnection: `Bạn sẽ cảm thấy hài lòng khi sử dụng ${productName}!`,
        callToAction: 'Đặt hàng ngay hôm nay!',
        urgencyFactor: 'Ưu đãi có hạn, nhanh tay đặt hàng!'
      }
    }
...
Tạo đoạn thoại ĐỘCĐÁO, KHÔNG TRÙNG LẶP!`
    ...
    console.error("Error in generate-dialogue API:", error)

    return Response.json({
      error: "Có lỗi xảy ra khi tạo đoạn thoại",
